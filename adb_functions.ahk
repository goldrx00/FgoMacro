global hBitmap := 0
global ADB_TIME_REFRESH := 400

getAdbScreen() ;;adb에서 화면 가져와서 hBitmap에 저장
{
	if(hBitmap)
	{
		DllCall("DeleteObject", Ptr, hBitmap) ;;이전 hBitmap 주소의 메모리를 비운다. 메모리 누수를 막기 위함.
	}
...
옵지이미지변환(hbm)
{
	...
	hbm2 := Gdip_CreateHBITMAPFromBitmap(ret2)
...
IsImageWithoutCap(ByRef clickX, ByRef clickY, ImageName, errorRange, trans, sX = 0, sY = 0, eX = 0, eY = 0) ;gdip
{
	...
	If(Gdip_ImageSearchWithHbm(hBitmap, ClickX, ClickY, ImageName, errorRange, trans, sX, sY, eX, eY))
    {
		log := "  @ 이미지 찾음 : " ImageName
		AddLog(log)	
        return true
